...
var key = 'innjia20180329' //innjia2016111703
...
function getToken (methodId) {
  var user = localStorage.getItem('user')
  if (user) {
    user = JSON.parse(user)
    if (!isZGToken(methodId)){
      return user.userId + ':' + user.innJiaToken
    }else {
...
function isZGToken(methodId) {
  let appArr = ['N002','N003','N006','1004','1005']
  if (appArr.indexOf(methodId) != -1){
    return false
  }else {
    return true
  }
...
var rest = function (opt) {
  ...
  return $.ajax({
    ...
    type: 'post',
    // headers: {Authorization: tokenFlag},
    success: opt.success,
    error: opt.error,
...
class mySign {
  rest (obj) {
    return function (opt) {
      ...
      var tokenFlag = getToken(opt.methodId)
      return $.ajax({
        url: process.env.API_HOST,
        data: signature(opt.methodId, opt.data),
        type: 'post',
